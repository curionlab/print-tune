...
from printtune.core.imaging.upload import process_uploaded_image, save_uploaded_image_to_temp
...
if "use_sample" not in st.session_state:
    st.session_state.use_sample = False  # デフォルトはsample.pngを使わない

# 画像アップロードUI（セッション開始前またはセッション中でも変更可能）
st.subheader("画像の選択")
use_sample = st.toggle("sample.png を使う", value=st.session_state.use_sample, key="use_sample_toggle")
...
if not use_sample:
    uploaded_file = st.file_uploader(
        "写真をアップロード（JPEG/PNG対応）",
        type=["jpg", "jpeg", "png"],
        key="image_uploader"
    )
    if uploaded_file is not None:
        # アップロード画像を処理してsession_stateに保存
        img_processed = process_uploaded_image(uploaded_file)
        st.session_state.uploaded_image = img_processed
        # 元のファイル名を保存（拡張子を除く）
        from pathlib import Path
        original_filename = Path(uploaded_file.name).stem
        st.session_state.uploaded_image_filename = original_filename
        # 画像をアップロードしたら、use_sampleをFalseに設定
...
if st.button("Start new session"):
    # セッションIDを先に生成
    from printtune.core.ids import SessionId
    temp_sid = SessionId.new()
    
    # 使用する画像のパスを決定
    original_filename = "sample"
    if not use_sample and st.session_state.uploaded_image is not None:
        # アップロード画像を使用（一時ファイルとして保存）
        temp_path = save_uploaded_image_to_temp(st.session_state.uploaded_image, temp_sid.value)
        image_path = str(temp_path)
        # 元のファイル名を取得
        original_filename = st.session_state.get("uploaded_image_filename", "uploaded")
    else:
        # sample.pngを使用
        image_path = "data/input/sample.png"
        original_filename = "sample"
    
    sess = new_session(sample_image_relpath=image_path)
    # 元のファイル名をsession_stateに保存（セッションIDをキーに）
    st.session_state[f"original_filename_{sess.session_id}"] = original_filename
...
# 元のファイル名を取得
original_filename = st.session_state.get(f"original_filename_{sid}", "sample")
# セッションファイルから読み込んだ場合、session_stateにない可能性があるので、パスから推測
if original_filename == "sample" and sess.sample_image_relpath and sess.sample_image_relpath != "data/input/sample.png":
    path_obj = Path(sess.sample_image_relpath)
    # 一時ファイル名から元のファイル名を推測できないので、デフォルト値を使用
    # 将来的にはセッションファイルに保存することを検討
...
# Round sheetは既にPNGファイルなので、そのまま読み込んで表示
from PIL import Image as PILImage
sheet_img = PILImage.open(sheet_path)

# ファイル名を生成（元のファイル名 + round番号）
download_filename = f"{original_filename}_round{current.round_index}"

display_image_png(sheet_img, caption=f"Round{current.round_index} sheet (Print Candidates)", width="stretch", download_filename=download_filename)
...
if uploaded is not None:
    img_verify = process_uploaded_image(uploaded)
    # PNG形式を保持したまま表示
    display_image_png(img_verify, caption="Verify input", width="stretch")
    ...
    if bp_path.exists():
        ...
        # PNG形式を保持したまま表示
        display_image_png(out_img, caption="Verify output (current best)", width="stretch")
